import numpy as np
from vectors import unitvec, rotate_by_angles
...
def plot_azel(data, task, pd, resolution=100, cbar=True,
    ...
    if subplot_spec == None:
        ...
        h = 7
        if fig != None:
            warn("Ignoring supplied `fig` value because subplot_spec is None.")
        ...
        if fig == None:
            raise ValueError("You need to supply a `fig` argument "
                             "when subplot_spec is given.")
        m = ptl.subplot_spec2margins(subplot_spec, fig)
        ax_rects = ptl.get_ax_rects(np.arange(ntime * 2), 2, ntime,
                                    margin=m, direction='col')
        #gs = GridSpecFromSubplotSpec(ntime + extra, 2,
        #                             subplot_spec=subplot_spec)
        #gs.update(left=0.1, right=0.9, bottom=0.1, hspace=0.05, wspace=0.05)
        #gs.set_height_ratios([1,] * ntime + [0.3,])
