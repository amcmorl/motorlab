...
ro.r('source("fit_CV_allback.R")')
ro.r('source("simulate_realkinematics.R")')
fit_CV_allback = ro.r('fit_CV_allback')

noise = 0.0001
data = ro.r('simu.realpos(sd.factor=%0.4f)' % noise)
out = fit_CV_allback(data, 10)
sse_matrix = np.array([np.array(x) for x in list(out[0])])
preds = np.array([np.array(x) for x in list(out[1])])
actual = np.array([np.array(x) for x in list(out[2])])
n_mods = sse_matrix.shape[0]
#sse_matrix.tofile('cv_matrix_with_%0.0enoise.txt' % noise, sep=' ')
