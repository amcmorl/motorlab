...
fit_CV_kd = function (moddata, n.cv=10, spl.k=10) {
  ...
  for (j in 1:n.dataset) {
	...
	for ( i.cv  in 1:n.cv) {
      ...
      #fit for each model
      #kd 
      out.kd = gam(yvec~dx + dy + dz, data=DATA1, family="poisson")
	
      temp.predict.mat      = matrix(0,1,n.testpts)
  ...
  #The returned value
  list(PREDICT.ALL, ACTUAL.ALL, TRIALS.ALL, COEFS.ALL, COEFNAMES.ALL, list(NULL))
}
